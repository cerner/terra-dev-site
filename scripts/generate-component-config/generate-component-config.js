...
commander
  ...
  .option('--no-mono-repo', 'Disable the search patterns assumption of a mono repo project structure')
...
const packagesPath = commander.monoRepo ? path.resolve(process.cwd(), 'packages') : process.cwd();
...
glob(searchPath, { nodir: true }, (er, foundFiles) => {
  ...
  foundFiles.forEach((filePath) => {
    ...
    if (isExampleFile) {
      ...
      const importPath = path.join('..', 'packages', `${packageName}`, 'examples', `${fileName}.site-page`);
      exampleImports += `import ${importName} from '${importPath}';\n`;
    } else {
      importName = `${startCase(fileName)}`.replace(/\s/g, '');
      const importPath = path.join('..', 'packages', `${packageName}`, 'examples', 'test-examples', `${fileName}`);
  ...
  fs.writeFileSync(path.join(process.cwd(), 'site', 'terra-ui-config.json'), JSON.stringify(UIconfig, null, 2));
